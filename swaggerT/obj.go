...
var TypeMap = map[string]reflect.Type{
	"null.String": reflect.TypeOf(""),
	"null.Int":    reflect.TypeOf(0),
	"null.Float":  reflect.TypeOf(0.0),
	"null.Bool":   reflect.TypeOf(true),
	"null.Time":   reflect.TypeOf(time.Now()),
...
func (s Server) Http(r *http.Request, w http.ResponseWriter) error {
	...
	switch path {
	case "index.html":
		if err := index.Execute(w, &swaggerUIBundle{URL: fmt.Sprintf("http://%s/swagger/doc.json", s.Host)}); err != nil {
			return err
		}
	case "doc.json":
		if _, err := w.Write(s.doc); err != nil {
			return err
		}
	default:
		h.ServeHTTP(w, r)
...
func (p *Params) findType(t reflect.Type) reflect.Type {
...
func (p *Params) Scanner(t reflect.Type) {
	if t.Kind() == reflect.Ptr {
		t = t.Elem()
	}
	if p.Form == nil {
		p.Form = make(map[string]reflect.Type)
	}
	if p.Path == nil {
		p.Path = make(map[string]reflect.Type)
	}
	if p.Query == nil {
		p.Query = make(map[string]reflect.Type)
	}
	if p.Body == nil {
		p.Body = make(map[string]reflect.Type)
	}
	for i := 0; i < t.NumField(); i++ {
		...
		if json := field.Tag.Get("json"); json != "" {
			p.Body[json] = p.findType(ft)
		} else if query := field.Tag.Get("query"); query != "" {
			p.Query[query] = p.findType(ft)
		} else if form := field.Tag.Get("form"); form != "" {
			p.Form[form] = p.findType(ft)
		} else if path := field.Tag.Get("path"); path != "" {
			p.Path[path] = p.findType(ft)
