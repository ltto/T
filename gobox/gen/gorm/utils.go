...
func Generate(columnTypes map[string]map[string]string, tableName string, structName string, pkgName string, jsonAnnotation bool, gormAnnotation bool, gureguTypes bool) ([]byte, error) {
	...
	src := fmt.Sprintf(head, pkgName, structName, dbTypes)
...
func ScanTable(param *Param) ([]byte, error) {
	var result string
	i := 0
	for table, struct_ := range param.Table2struct {
		port, _ := strconv.Atoi(param.Port)
		columnDataTypes, err := GetColumnsFromMysqlTable(param.User, param.Password, param.Host, port, param.Database, table)
		if err != nil {
			return []byte{}, err
		}
		struc, err := Generate(*columnDataTypes, table, struct_, param.Package, param.JsonAble, param.GormAble, false)
		if i > 0 {
			struc = bytes.TrimLeft(struc, fmt.Sprintf("package %s", param.Package))
		}
	...
	source, err := format.Source([]byte(result))
	if err != nil {
